 ...
 * platforms.
 * <p>
 * The 32 bit x86 version of this hash should be the fastest variant for relatively short keys
 like ids.
 * murmurhash3_x64_128 is a good choice for longer strings or if you need more than 32 bits of
 hash.
 * <p>
...
interface MurmurHash3 {
  ...
  /** Returns the MurmurHash3_x86_32 hash of the UTF-8 bytes of the String without actually
   encoding
  ...
  public static int murmurhash3_x86_32(CharSequence data, int offset, int len, int seed) {
    ...
    while (pos < end) {
      ...
      if (code < 0x80) {
        ...
        k2 =
            (0xE0 | (code >> 12)) | ((0x80 | ((code >> 6) & 0x3F)) << 8)
                | ((0x80 | (code & 0x3F)) << 16);
        bits = 24;
      } else {
        // surrogate pair
