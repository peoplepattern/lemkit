 ...
 */
class ClassifierIndexer(val lmap: Map[String, Int], val fmap: FeatureMap) {
  def apply(features: FeatureSet[String]): FeatureSet[Int] = {
    ...
    condense(
      (FeatureObservation("", 1.0) +: features)
...
object ClassifierIndexer {
  ...
  def index[T](examples: TraversableOnce[Example[String, String]],
  ...
  def indexExact[T](
    ...
    val indexedExamples =
      indexExamples(examples, lmap, x => x.map(feature => feature.map(fmap)))

    val (cooked, numExamples) = process(indexedExamples)

    (cooked,
      new ClassifierIndexer(lmap.toMap, new ExactFeatureMap(fmap.toMap)),
      numExamples)
  }
   ...
   */
  def indexHashed[T](examples: TraversableOnce[Example[String, String]],
    process: TraversableOnce[Example[Int, Int]] => (T, Int),
    maxNumFeatures: Int) = {
    ...
    val indexedExamples = indexExamples(
      examples,
      lmap,
