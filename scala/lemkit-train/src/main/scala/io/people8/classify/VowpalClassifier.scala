...
object VowpalClassifier extends LinearClassifierTrainer {
  ...
  def train[I](
    ...
    val filteredFmapOpt = if (useHashtrick || !doFeatureSelection) None else {
      ...
      while (numIters < 10 && npUpper > upperbound && npLower < lowerbound) {
        ...
        val trainL1Command = s"vw -d $trainingFile --oaa $numClasses --passes 50 --loss_function logistic --l1 $l1reg --noconstant --readable_model $paramsL1File --cache_file $cacheFile -k $quietOption -b $numBits"
        ...
        if (options.verbose) {
          println("\n***********************************************")
          println(s"L1 feature selection (Iter $numIters): Lower $npLower\tUpper $npUpper\tL1-curr $l1Curr\tL1-lower $l1Lower\t L1-upper $l1Upper\n")
      ...
      // Train the model given the current L1 regularization parameter.
      val trainL1Command = s"vw -d $trainingFile --oaa $numClasses --passes 50 --loss_function logistic --l1 $l1FinalExp --noconstant --readable_model $paramsL1File --cache_file $cacheFile -k $quietOption -b $numBits"
      ...
      val activeFeatureIndices = getActiveFeatures(paramsL1File, numClasses).toSeq
      ...
      val activeFeatureNames = activeFeatureIndices.map(reverseFullFmap).toSet
      val activeFeatures =
      ...
      val fmask = (for ((feature, index) <- activeFeatures.zipWithIndex) yield (rawFmap(feature), index)).toMap
    ...
    val trainCommand = s"vw -d $trainingFileFinal --oaa $numClasses --l2 ${options.regularization} --noconstant --readable_model $paramsFile --cache_file $cacheFile -k $quietOption -b $numBits ${options.defaultOptions} ${options.extraOptions}"
  ...
  private def getNumBitsNeeded(numFeatures: Int, numClasses: Int) = {
    val numBits = ceil(log(numFeatures) / log(2)).toInt + getBitShift(numClasses)
    assert(numBits < 33)
    numBits
  }
   ...
   */
  private def readParameters(paramsFile: File, numClasses: Int, numFeatures: Int) = {
    ...
    for {
      ...
      (parameterIndex, classIndex) = getOriginalIndices(indexString.toInt, bitsToShift)
    } {
